...
class HFResidualBlock(nn.Module):
    ...
    def forward(self, x, conditioner, diffusion_step, conditioner_global=None):
        ...
        y = self.output_projection(y)
        residual, skip = torch.chunk(y, 2, dim=1)
        return (x + residual) / sqrt(5.0), skip

class HifiDiffV18R21(nn.Module):
    def __init__(self, params):
        ...
        self.spectrogram_upsampler = SpectrogramUpsampler(self.n_mels, periodic=True)
        ...
        self.hf_residual_layers = nn.ModuleList([
            HFResidualBlock(self.n_mels, params.residual_channels, 2 ** (i % 8),
                          n_cond_global=self.n_cond)
            for i in range(params.residual_layers)
        ])
        self.lf_residual_layers = nn.ModuleList([
            LFResidualBlock(self.n_mels, params.residual_channels, 2 ** (i % 14),
                          n_cond_global=self.n_cond)
            for i in range(params.residual_layers)
        ])
